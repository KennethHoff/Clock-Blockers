...
namespace Sisus
{
	...
	public class InspectorManager : Singleton<InspectorManager>, IInspectorManager, IDisposable
	{
		...
		public void OnLayout()
		{
			...
			if(onNextLayout != null)
			{
				...
				for(int n = 0, count = invocationList.Length; n < count; n++)
				{
					...
					{
						#if DEV_MODE
						if(!ExitGUIUtility.IsExitGUIException(exception)) { Debug.LogError(e); }
						#endif
		...
		public void EnsureOnGUICallbacks(bool needsLayoutEvent)
		{
			...
			#endif			
			{
				DrawGUI.EnsureOnGUICallbackOnExistingInspector();
			}
		}
		...
		private void OnKeyDown(Event e)
		{
			...
			{
				selectedInspector.InspectorDrawer.OnKeyDown(e);
			}

			for(int n = activeInstances.Count - 1; n >= 0; n--)
			{
				var activeInstance = activeInstances[n];
				if(activeInstance != selectedInspector)
				{
					activeInstance.InspectorDrawer.OnKeyDown(e);
				}
			}
		}
		...
		{
			var editorWindows = Resources.FindObjectsOfTypeAll<UnityEditor.EditorWindow>();
			int foundCount = 0;
