...
namespace Sisus
{
	...
	public static class InspectorUtility
	{
		...
		/// <summary>
		/// InspectorManager which is now "active", i.e. currently being drawn, edited or interacted with in some way.
		/// Note that this is not cleared at the end of each OnGUI call, so it can also return the last active manager,
		/// even if it's not active in any way any longer.
		/// </summary>
		/// <value>
		/// The current or last active inspector manager.
		/// </value>
		...
		/// <summary>
		/// Inspector drawer which is now "active", i.e. currently being drawn, edited or interacted with in some way.
		...
		public static bool IsSafeToChangeInspectorContents
		{
			get
			{
				return NowDrawingInspectorPart == InspectorPart.None && Event.current != null;
			}
		}
