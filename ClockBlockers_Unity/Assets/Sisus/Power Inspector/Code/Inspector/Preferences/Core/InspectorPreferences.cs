		...
		public bool EnableCategorizedComponents
 		{
			...
			set
			{
				...
				var inspectorTypes = TypeExtensions.GetImplementingNonUnityObjectClassTypes(typeof(IInspector), false);
				for(int n = inspectorTypes.Length - 1; n >= 0; n--)
				{
					var inspectorType = inspectorTypes[n];
					if(inspectorType.IsAbstract)
					{
						continue;
					}

					var drawerProvider = DefaultDrawerProviders.GetForInspector(inspectorType) as DrawerProviderBase;
					if(drawerProvider != null)
					{
						drawerProvider.drawersFor.gameObject = categorizedComponents ? typeof(CategorizedGameObjectDrawer) : typeof(GameObjectDrawer);
					}
				}
