...
namespace Sisus
{
	...
	public abstract class FormattedTextAssetDrawer<TFormatter> : CustomEditorAssetDrawer, IScrollable where TFormatter : ITextSyntaxFormatter, new()
	{
		...
		#endif
		private static readonly StringBuilder sb = new StringBuilder(10000);
		...
		/// <summary>
		/// Called when user pressed the a mouse button down over a line in the text editor.
		/// </summary>
		...
		private void OnLineClicked(int index, bool lineNumberClicked)
		{
			...
			switch(Event.current.button)
			{
				...
				case 1:
					...
					if(AllowEditing)
					{
						menu.Add("Delete Row", ()=>code.RemoveAt(index));
						menu.Add("Insert Row", ()=>code.InsertAt(index, ""));
						menu.Add("Insert Summary", ()=>
						{
							...
							code.InsertAt(index, string.Concat(leadingWhiteSpace, "/// </summary>"));
							code.InsertAt(index, string.Concat(leadingWhiteSpace, "/// "));
							code.InsertAt(index, string.Concat(leadingWhiteSpace, "/// <summary>"));
			...
			switch(inputEvent.keyCode)
			{
				...
				case KeyCode.KeypadEnter:
					if(inputEvent.shift || inputEvent.control)
					{
						if(selectedLine >= 0)
						{
							...
							code.InsertAt(inputEvent.shift ? selectedLine : selectedLine + 1, ""); //shift+enter adds above, ctrl+enter adds below
							SetHasUnsavedChanges(true);
				...
				case KeyCode.Delete:
					if((inputEvent.modifiers == EventModifiers.Shift || inputEvent.modifiers == EventModifiers.Control) && selectedLine >= 0)
					{
						...
						if(code.LineCount <= 1)
						{
							code.SetLine(selectedLine, "");
						}
						else
						{
							code.RemoveAt(selectedLine);
							UpdateVisibleLines();
						}
