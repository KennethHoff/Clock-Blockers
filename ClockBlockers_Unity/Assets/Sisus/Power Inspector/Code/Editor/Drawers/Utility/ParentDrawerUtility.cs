	...
	public static class ParentDrawerUtility
	{
		...
		public static void GetMemberBuildList([NotNull]IParentDrawer parent, [NotNull]LinkedMemberHierarchy hierarchy, [NotNull]ref List<LinkedMemberInfo> results, DebugModeDisplaySettings debugModeSettings, BindingFlags bindingFlags = BindingFlagsInstanceAndStatic)
		{
			...
			if(debugModeSettings.Static)
			{
				if(debugModeSettings.ShowFields)
				{
					type.GetInspectorViewableFieldsInStaticMode(ref hierarchy, parentMemberInfo, ref results, true, FieldVisibility.AllExceptHidden, PropertyVisibility.AllPublic, MethodVisibility.AttributeExposedOnly);
				}

				if(debugModeSettings.ShowProperties)
				{
					type.GetInspectorViewablePropertiesInStaticMode(ref hierarchy, parentMemberInfo, ref results, true, FieldVisibility.AllExceptHidden, PropertyVisibility.AllPublic, MethodVisibility.AttributeExposedOnly);
				}

				if(debugModeSettings.ShowMethods)
				{
					type.GetInspectorViewableMethodsInStaticMode(ref hierarchy, parentMemberInfo, ref results, true, MethodVisibility.AllPublic);
				}
			}
			else
			{
