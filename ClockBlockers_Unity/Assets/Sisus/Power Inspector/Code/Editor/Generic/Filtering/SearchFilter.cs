...
namespace Sisus
{
	...
	public class SearchFilter
	{
		...
		public bool SetFilter([NotNull]string setFilterValue, [CanBeNull]IInspector inspector, [CanBeNull]Action<SearchFilter> onFilterChanging, [CanBeNull]Action onFilterChanged)
		{
			...
			#if DEV_MODE
			UnityEngine.Debug.Log("SearchFilter = \"" + setFilterValue + "\" (was : \"" + rawInput + "\")");
			#endif
				...
				for(int start = 0, end = filterLower.IndexOf(' '); start < charCount; start = end + 1)
				{
					...
					if(quotationMark != -1 && quotationMark < end)
					{
						...
						if(secondQuotationMark != -1)
						{
							...
							#if DEV_MODE
							UnityEngine.Debug.Log("\""+s+"\" with start="+start+", end="+end+ ", quotationMark="+ quotationMark+ ", secondQuotationMark="+ secondQuotationMark+", charCount = " + charCount+ ", filterLower="+ filterLower);
							#endif
						}
					}
							...
							switch(s[0])
							{
								case 't': // type
									...
									if(length > 2)
									{
										...
										hasTypeFilter = true;
										//hasFilter = true; // should we set has filter true in this case? the filter is just used set the target, not to filter the contents of the inspector itself
								...
								case 'c': // class
									...
									if(inspector != null && filterLower.StartsWith("c:", StringComparison.OrdinalIgnoreCase) && inspector.GetType() != typeof(PreferencesInspector))
									{
										...
										{
											#if DEV_MODE
											UnityEngine.Debug.Log("Inspecting static members for "+StringUtils.ToString(classType));
											#endif
											inspector.RebuildDrawers(null, classType);
										}
										#if DEV_MODE
										else { UnityEngine.Debug.Log("Failed to find class type from string "+StringUtils.ToString(className)); }
										#endif
									}
									break;
								#if UNITY_EDITOR
								...
								case 's': // select scene objects
									if(inspector != null && filterLower.StartsWith("s:", StringComparison.OrdinalIgnoreCase) && inspector.GetType() != typeof(PreferencesInspector))
									{
										...
										if(classType != null)
										{
											...
											{
												#if DEV_MODE
												UnityEngine.Debug.Log("Selecting instances of Component "+StringUtils.ToString(classType)+ " in scene.");
												#endif
								...
								case 'i': // icons
									if(inspector != null && inspector.GetType() != typeof(PreferencesInspector))
									{
										...
										if(iconFilter.Length > 0)
										{
											...
											{
												var content = UnityEditor.EditorGUIUtility.ObjectContent(null, types[n]);
											...
											var allTextures = UnityEngine.Resources.FindObjectsOfTypeAll<UnityEngine.Texture>();
									...
									if(inspector != null && filterLower.StartsWith("g:", StringComparison.OrdinalIgnoreCase) && inspector.GetType() != typeof(PreferencesInspector))
									{
										...
										if(filterText.Length > 0)
										{
											...
											if(found)
											{
												...
												#if DEV_MODE
												UnityEngine.Debug.Log("previewText="+ previewText+", style="+style.name);
												#endif
										...
										if(window != null)
										{
											...
											if(x != -1)
											{
												...
												if(float.TryParse(s.Substring(x+1), out height) && height >= 50f)
												{
													...
													{
														UnityEngine.Debug.LogWarning("RESIZING INSPECTOR WINDOW TO: "+width+"x"+height);
											...
											else
											{
												...
												{
													UnityEngine.Debug.LogWarning("RESIZING INSPECTOR WINDOW TO WIDTH: "+width);
													var pos = window.position;
													pos.width = width;
													window.position = pos;
												}
		...
		public bool PassesFilter(IUnityObjectDrawer subject)
		{
			...
			if(hasTypeFilter)
			{
				...
				#endif
				{
					...
					{ UnityEngine.Debug.Log(subject.ToString()+" - PassesFilter(\"" + rawInput + "\"): "+StringUtils.True); }
					#endif
		...
		private static bool PassesTypeFilter(IDrawer subject, string filter, bool requireExactMatch)
		{
			if(filter.Length == 0)
			{
				#if DEV_MODE
				UnityEngine.Debug.LogWarning("PassesTypeFilter("+subject+") called with an empty filter string. Returning true.");
				#endif
				return true;
			}
			...
			//e.g. "unity.object" matches UnityEngine.Object, "sys.obj" matches "System.Object"
			...
			if(namespaceSeparator != -1)
			{
				if(requireExactMatch)
				{
					if(typeName.Equals(filter.Substring(namespaceSeparator + 1), StringComparison.OrdinalIgnoreCase) && type.Namespace.Equals(filter.Substring(0, namespaceSeparator), StringComparison.OrdinalIgnoreCase))
					{
						...
						if(string.IsNullOrEmpty(OnlyDebugFieldByName) || string.Equals(subject.Name, OnlyDebugFieldByName, StringComparison.OrdinalIgnoreCase))
						{ UnityEngine.Debug.Log(subject.FullClassName+ " (" + StringUtils.ToString(subject.Type) + ") - PassesTypeFilter(\"" + filter + "\"): "+StringUtils.True); }
						#endif
			...
			if(string.IsNullOrEmpty(OnlyDebugFieldByName) || string.Equals(subject.Name, OnlyDebugFieldByName, StringComparison.OrdinalIgnoreCase))
			{ UnityEngine.Debug.Log(subject.FullClassName+ " (" + StringUtils.ToString(subject.Type) + ") - PassesTypeFilter(\"" + filter + "\"): "+StringUtils.False); }
			#endif
		...
		public bool PassesFilter(UnityEditor.SerializedProperty property, ref FilterTestType passedTestMethod)
		{
			...
			{ UnityEngine.Debug.Log(property.propertyPath+ " (" + property.type + ") - PassesFilter(\"" + rawInput + "\"): "+StringUtils.True); }
			#endif
		...
		private static bool PassesTypeFilter(string typeName, string filter, bool requireExactMatch)
		{
			...
			{
				#if DEV_MODE
				UnityEngine.Debug.LogWarning("PassesTypeFilter("+typeName+") called with an empty filter string. Returning true.");
				#endif
				return true;
			}
			...
			if(requireExactMatch)
			{
				...
				{
					#if DEV_MODE && DEBUG_PASSES_SEARCH_FILTER
					UnityEngine.Debug.Log(StringUtils.ToString(typeName) + " - PassesTypeFilter(\"" + filter + "\"): "+StringUtils.True);
					#endif
			...
			#if DEV_MODE && DEBUG_FAILS_SEARCH_FILTER
			UnityEngine.Debug.Log(StringUtils.ToString(typeName) + " - PassesTypeFilter(\"" + filter + "\"): "+StringUtils.False);
			#endif
		...
		public bool PassesFilter(IDrawer subject, out FilterTestType passedTestMethod)
		{
			...
			if(hasLabelFilter)
			{
				...
				UnityEngine.Debug.Assert(passedTestMethod.HasFlag(FilterTestType.Label) || passedTestMethod.HasFlag(FilterTestType.FullClassName));
				#endif
			}
			...
			#if DEV_MODE && DEBUG_PASSES_SEARCH_FILTER_BASIC
			UnityEngine.Debug.Log(subject.FullClassName+ " (" + StringUtils.ToString(subject.Type) + ") - PassesFilter(\"" + rawInput + "\"): "+StringUtils.True);
			#endif
		...
		private static bool PassesLabelFilter(IDrawer subject, string filter, bool requireExactMatch, ref FilterTestType passedTestMethod)
		{
			...
			{
				#if DEV_MODE
				UnityEngine.Debug.LogWarning("PassesLabelFilter("+subject+") called with an empty filter string. Returning true.");
				#endif
				return true;
			}
		...
		public static bool PassesLabelFilter(string fullName, string displayName, string filter, bool requireExactMatch, ref FilterTestType passedTestMethod)
		{
			...
			{
				#if DEV_MODE
				UnityEngine.Debug.LogWarning("PassesLabelFilter("+fullName+") called with an empty filter string. Returning true.");
				#endif
				return true;
			}
			...
			if(requireExactMatch)
			{
				if(fullName.Equals(filter, StringComparison.OrdinalIgnoreCase))
				{
					...
					{ UnityEngine.Debug.Log(fullName + ") - PassesLabelFilter(\"" + filter + "\"): "+StringUtils.True+ " (via fullName)"); }
					#endif
					return true;
				}
			}
			...
			if(requireExactMatch)
			{
				if(displayName.Length != fullName.Length && displayName.Equals(filter, StringComparison.OrdinalIgnoreCase))
				{
					#if DEV_MODE && DEBUG_PASSES_SEARCH_FILTER
					if(string.IsNullOrEmpty(OnlyDebugFieldByName) || string.Equals(displayName, OnlyDebugFieldByName, StringComparison.OrdinalIgnoreCase))
					{ UnityEngine.Debug.Log(fullName + ") - PassesLabelFilter(\"" + filter + "\"): "+StringUtils.True+ " (via display name)"); }
					#endif
					return true;
				}
			}
			...
			#if DEV_MODE && DEBUG_FAILS_SEARCH_FILTER
			if(string.IsNullOrEmpty(OnlyDebugFieldByName) || string.Equals(displayName, OnlyDebugFieldByName, StringComparison.OrdinalIgnoreCase))
			{ UnityEngine.Debug.Log(fullName + ") - PassesLabelFilter(\"" + filter + "\"): "+StringUtils.False+ " (full name and display failed test)"); }
			#endif
			return false;
		}
		...
		private static bool TryTestValueFilter(IDrawer subject, string filter, bool requireExactMatch, out bool result)
		{
			...
			if(contentString == null)
			{
				#if DEV_MODE && DEBUG_FAILS_SEARCH_FILTER
				if(string.IsNullOrEmpty(OnlyDebugFieldByName) || string.Equals(subject.Name, OnlyDebugFieldByName, StringComparison.OrdinalIgnoreCase))
				{ UnityEngine.Debug.Log(subject.FullClassName+ " (" + StringUtils.ToString(subject.Type) + ") - PassesValueFilter(\"" + filter + "\"): "+StringUtils.Green("n/a")+ " (ValueToStringForFiltering returned null)"); }
				#endif
				result = true;
				return false;
			}
			...
			if(requireExactMatch)
			{
				if(contentString.Equals(filter, StringComparison.OrdinalIgnoreCase))
				{
					#if DEV_MODE && DEBUG_PASSES_SEARCH_FILTER
					if(string.IsNullOrEmpty(OnlyDebugFieldByName) || string.Equals(subject.Name, OnlyDebugFieldByName, StringComparison.OrdinalIgnoreCase))
					{ UnityEngine.Debug.Log(subject.FullClassName+ " (" + StringUtils.ToString(subject.Type) + ") - PassesValueFilter(\"" + filter + "\"): "+StringUtils.True+ " (\"" + contentString + "\" did not contain filter)"); }
					#endif
			...
			#if DEV_MODE && DEBUG_FAILS_SEARCH_FILTER
			if(string.IsNullOrEmpty(OnlyDebugFieldByName) || string.Equals(subject.Name, OnlyDebugFieldByName, StringComparison.OrdinalIgnoreCase))
			{ UnityEngine.Debug.Log(subject.FullClassName+ " (" + StringUtils.ToString(subject.Type) + ") - PassesValueFilter(\"" + filter + "\"): "+StringUtils.False+ " (\""+contentString+ "\" did not contain filter)"); }
			#endif
			result = false;
			return true;
		}
		...
		private static bool PassesValueFilter(IDrawer subject, string filter, bool requireExactMatch)
		{
			...
			{
				#if DEV_MODE
				UnityEngine.Debug.LogWarning("PassesValueFilter("+subject+") called with an empty filter string. Returning true.");
				#endif
				return true;
			}
			...
			if(contentString == null)
			{
				#if DEV_MODE && DEBUG_PASSES_SEARCH_FILTER
				if(string.IsNullOrEmpty(OnlyDebugFieldByName) || string.Equals(subject.Name, OnlyDebugFieldByName, StringComparison.OrdinalIgnoreCase))
				{ UnityEngine.Debug.Log(subject.FullClassName+ " (" + StringUtils.ToString(subject.Type) + ") - PassesValueFilter(\"" + filter + "\"): "+StringUtils.True+ " (ValueToStringForFiltering returned null)"); }
				#endif
				return true;
			}
			...
			if(requireExactMatch)
			{
				if(contentString.Equals(filter, StringComparison.OrdinalIgnoreCase))
				{
					#if DEV_MODE && DEBUG_PASSES_SEARCH_FILTER
					if(string.IsNullOrEmpty(OnlyDebugFieldByName) || string.Equals(subject.Name, OnlyDebugFieldByName, StringComparison.OrdinalIgnoreCase))
					{ UnityEngine.Debug.Log(subject.FullClassName+ " (" + StringUtils.ToString(subject.Type) + ") - PassesValueFilter(\"" + filter + "\"): "+StringUtils.True+ " (\"" + contentString + "\" contained filter)"); }
					#endif
		...
		public static bool PassesGenericFilter(IDrawer subject, string filter, bool requireExactMatch, ref FilterTestType passedTestMethod)
		{
			...
			{
				#if DEV_MODE
				UnityEngine.Debug.LogWarning("PassesGenericFilter("+subject+") called with an empty filter string. Returning true.");
				#endif
				return true;
			}
			...
			if(PassesLabelFilter(subject, filter, requireExactMatch, ref passedTestMethod))
			{
				#if DEV_MODE && DEBUG_PASSES_SEARCH_FILTER
				if(string.IsNullOrEmpty(OnlyDebugFieldByName) || string.Equals(subject.Name, OnlyDebugFieldByName, StringComparison.OrdinalIgnoreCase))
				{ UnityEngine.Debug.Log(subject.FullClassName+ " (" + StringUtils.ToString(subject.Type) + ") - PassesGenericFilter(\"" + filter + "\"): "+StringUtils.True+ " (via label)"); }
				#endif
				return true;
			}
			...
			if(PassesTypeFilter(subject, filter, requireExactMatch))
			{
				...
				#if DEV_MODE && DEBUG_PASSES_SEARCH_FILTER
				if(string.IsNullOrEmpty(OnlyDebugFieldByName) || string.Equals(subject.Name, OnlyDebugFieldByName, StringComparison.OrdinalIgnoreCase))
				{ UnityEngine.Debug.Log(subject.FullClassName+ " (" + StringUtils.ToString(subject.Type) + ") - PassesGenericFilter(\"" + filter + "\"): "+StringUtils.True+ " (via type)"); }
				#endif
				return true;
			}
			...
			if(TryTestValueFilter(subject, filter, requireExactMatch, out passedValueTest))
			{
				if(passedValueTest)
				{
					...
					#if DEV_MODE && DEBUG_PASSES_SEARCH_FILTER
					if(string.IsNullOrEmpty(OnlyDebugFieldByName) || string.Equals(subject.Name, OnlyDebugFieldByName, StringComparison.OrdinalIgnoreCase))
					{ UnityEngine.Debug.Log(subject.FullClassName+ " (" + StringUtils.ToString(subject.Type) + ") - PassesGenericFilter(\"" + filter + "\"): "+StringUtils.True+ " (via value)"); }
					#endif
					return true;
				}
				...
				#if DEV_MODE && DEBUG_FAILS_SEARCH_FILTER
				if(string.IsNullOrEmpty(OnlyDebugFieldByName) || string.Equals(subject.Name, OnlyDebugFieldByName, StringComparison.OrdinalIgnoreCase))
				{ UnityEngine.Debug.Log(subject.FullClassName+ " (" + StringUtils.ToString(subject.Type) + ") - PassesGenericFilter(\"" + filter + "\"): "+StringUtils.False+ " (via value)"); }
				#endif
				return false;
			}
			...
			#if DEV_MODE && DEBUG_FAILS_SEARCH_FILTER
			if(string.IsNullOrEmpty(OnlyDebugFieldByName) || string.Equals(subject.Name, OnlyDebugFieldByName, StringComparison.OrdinalIgnoreCase))
			{ UnityEngine.Debug.Log(subject.FullClassName+ " (" + StringUtils.ToString(subject.Type) + ") - PassesGenericFilter(\"" + filter + "\"): "+StringUtils.False); }
			#endif
			return false;
		}
