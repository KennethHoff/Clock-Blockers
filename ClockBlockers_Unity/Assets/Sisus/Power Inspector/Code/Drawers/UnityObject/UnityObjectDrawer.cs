...
namespace Sisus
{
	...
	public abstract class UnityObjectDrawer<TSelf, TTarget> : ParentDrawer<LinkedMemberInfo>, IUnityObjectDrawer, IDebuggable where TSelf : UnityObjectDrawer<TSelf, TTarget> where TTarget : Object
	{
		...
		/// <summary> The fold arrow position. </summary>
		private Rect foldArrowPosition;

		/// <summary> The enabled flag position. </summary>
		private Rect enabledFlagPosition;

		/// <summary> The debug mode icon position. </summary>
		private Rect debugModeIconPosition;

		/// <summary> The execute method icon position. </summary>
		private Rect executeMethodIconPosition;

		/// <summary> The reference icon position. </summary>
		private Rect referenceIconPosition;

		#if UNITY_2018_1_OR_NEWER && UNITY_EDITOR
		/// <summary> The preset icon position. </summary>
		private Rect presetIconPosition;
		#endif

		/// <summary> The context menu icon position. </summary>
		private Rect contextMenuIconPosition;
		...
		/// <summary> The leftmost prefix icon position. </summary>
		private Rect leftmostPrefixIconPosition;
		...
		/// <summary> Gets a value indicating whether this object is currently editable. </summary>
		/// <value> True if editable, false if not. </value>
		...
		/// <summary> Gets the fold arrow position. </summary>
		/// <value> The fold arrow position. </value>
		protected Rect FoldArrowPosition
		{
			get
			{
		...
		/// <summary> Gets the debug mode icon position. </summary>
		/// <value> The debug mode icon position. </value>
		protected Rect DebugModeIconPosition
		{
			get
			{
				return debugModeIconPosition;
			}
		}
		...
		protected Rect ExecuteMethodIconPosition
		{
			get
			{
				return executeMethodIconPosition;
			}
		}

		/// <summary> Gets the reference icon position. </summary>
		/// <value> The reference icon position. </value>
		protected Rect ReferenceIconPosition
		{
			get
			{
				return referenceIconPosition;
			}
		}

		#if UNITY_2018_1_OR_NEWER && UNITY_EDITOR
		/// <summary> Gets the preset icon position. </summary>
		/// <value> The preset icon position. </value>
		protected Rect PresetIconPosition
		{
			get
			{
				return presetIconPosition;
			}
		}
		#endif

		/// <summary> Gets the context menu icon position. </summary>
		/// <value> The context menu icon position. </value>
		protected Rect ContextMenuIconPosition
		{
			get
			{
				return contextMenuIconPosition;
			}
		}
		...
		/// <summary> Gets the width of the context menu icon. </summary>
		/// <value> The width of the context menu icon. </value>
		private float ContextMenuIconWidth
		{
			get
			{
				return HeaderToolbarIconWidth;
			}
		}
		
		/// <summary> Gets the width of the preset icon. </summary>
		/// <value> The width of the preset icon. </value>
		private float PresetIconWidth
		{
			get
			{
				return HeaderToolbarIconWidth;
		...
		/// <summary> Gets the width of the reference icon. </summary>
		/// <value> The width of the reference icon. </value>
		private float ReferenceIconWidth
		{
			get
			{
				return HeaderToolbarIconWidth;
		...
		/// <summary> Gets the width of the debug mode icon. </summary>
		/// <value> The width of the debug mode icon. </value>
		private float DebugModeIconWidth
		{
			get
			{
				return HeaderToolbarIconWidth;
		...
		/// <summary> Gets the width of the execute method icon. </summary>
		/// <value> The width of the execute method icon. </value>
		private float ExecuteMethodIconWidth
		{
			get
			{
				return HeaderToolbarIconWidth;
		...
		/// <summary> Gets the leftmost prefix icon position. </summary>
		/// <value> The leftmost prefix icon position. </value>
		protected virtual Rect LeftmostPrefixIconPosition
		{
			get
			{
				return leftmostPrefixIconPosition;
			}
		}
		...
		protected override void GetDrawPositions(Rect position)
		{
			...
			else
			{
				...
				foldArrowPosition = labelLastDrawPosition;
				foldArrowPosition.y += 1f;
				foldArrowPosition.width = 18f;

				enabledFlagPosition = labelLastDrawPosition;
				...
				var nextIconRect = labelLastDrawPosition;
				nextIconRect.x = position.xMax - HeaderToolbarIconsRightOffset;
				nextIconRect.y += ToolbarIconsTopOffset;
				nextIconRect.width = ContextMenuIconWidth;
				nextIconRect.height = HeaderToolbarIconHeight;
				
				if(HasContextMenuIcon)
				{
					nextIconRect.x -= ContextMenuIconWidth;
					contextMenuIconPosition = nextIconRect;
					contextMenuIconPosition.width = ContextMenuIconWidth;
					nextIconRect.x -= HeaderToolbarIconsOffset;
				}
				
				prefixIconBackgroundPosition = nextIconRect;
				prefixIconBackgroundPosition.width = 0f;
				#if !UNITY_2019_3_OR_NEWER
				prefixIconBackgroundPosition.y += 1f;
				prefixIconBackgroundPosition.height -= 1f;
				#endif
				
				float offset = HeaderToolbarIconsOffset;

				#if UNITY_2018_1_OR_NEWER && UNITY_EDITOR
				if(HasPresetIcon)
				{
					float width = PresetIconWidth;
					nextIconRect.x -= width;
					presetIconPosition = nextIconRect;
					presetIconPosition.width = width;
					nextIconRect.x -= offset;

					// Don't increase bg width; we don't want to obscure the built-in preset icon.
					prefixIconBackgroundPosition.x -= width + offset;
				}
				#endif

				if(HasReferenceIcon)
				{
					float width = ReferenceIconWidth;
					nextIconRect.x -= width;
					referenceIconPosition = nextIconRect;
					referenceIconPosition.width = width;
					nextIconRect.x -= offset;

					// Don't increase bg width; we don't want to obscure the built-in reference icon.
					prefixIconBackgroundPosition.x -= width + offset;
				}

				if(HasExecuteMethodIcon)
				{
					float width = ExecuteMethodIconWidth;
					nextIconRect.x -= width;
					executeMethodIconPosition = nextIconRect;
					executeMethodIconPosition.width = width;
					nextIconRect.x -= offset;

					float adjustBg = width + offset;
					prefixIconBackgroundPosition.x -= adjustBg;
					prefixIconBackgroundPosition.width += adjustBg;
				}

				if(HasDebugModeIcon)
				{
					float width = DebugModeIconWidth;
					nextIconRect.x -= width;
					debugModeIconPosition = nextIconRect;
					debugModeIconPosition.width = width;
					nextIconRect.x -= offset;
				...
				nextIconRect.x += offset;
				leftmostPrefixIconPosition = nextIconRect;
			}
		...
		{
			if(labelLastDrawPosition.Contains(mousePos))
			{
		...
		protected virtual void DrawHeaderBase(Rect position)
		{
			...
			if(Event.current.type == EventType.Repaint || EditingComponentDisplayName)
			{
				...
				labelRect.width -= 55f + (position.width - leftmostPrefixIconPosition.x);
		...
		{
			if(mouseoveredPart != null && mouseoveredPart.OnRightClicked(inputEvent))
			{
				return true;
			}
		...
		protected virtual void DoBuildHeaderToolbar()
		{
			if(HasContextMenuIcon)
			{
				AddHeaderToolbarControl(HeaderPartDrawer.Create(HeaderPart.ContextMenuIcon, true, true, "Open context menu for target Object", OnContextMenuIconClicked));
			}

			#if UNITY_2018_1_OR_NEWER && UNITY_EDITOR
			if(HasPresetIcon)
			{
				AddHeaderToolbarControl(HeaderPartDrawer.Create(HeaderPart.PresetIcon, true, true, "Select Preset to use for target Object or save current state to a new Preset.", null));
			}
			#endif
			...
			if(HasReferenceIcon)
			{
				...
				if(docUrl.Length > 0)
				{
					AddHeaderToolbarControl(HeaderPartDrawer.Create(HeaderPart.ReferenceIcon, true, true, "Open documentation for "+title, null));
				}
				else
				{
					AddHeaderToolbarControl(HeaderPartDrawer.Create(HeaderPart.ReferenceIcon, true, true, "", OpenDocumentation));
				}
			}

			if(HasExecuteMethodIcon)
			{
				AddHeaderToolbarControl(HeaderPartDrawer.Create(HeaderPart.QuickInvokeMenuButton, true, true, InspectorUtility.Preferences.graphics.ExecuteIcon, "Invoke methods on target Object.\nRight-Click to also list invisible methods.", OnExecuteMethodIconClicked, OnExecuteMethodIconRightClicked));
			}
			
			if(HasDebugModeIcon)
			{
				AddHeaderToolbarControl(HeaderPartDrawer.Create(HeaderPart.DebugModePlusButton, true, true, debugMode ? InspectorUtility.Preferences.graphics.DebugModeOnIcon : InspectorUtility.Preferences.graphics.DebugModeOffIcon, debugMode ? "Disable Debug Mode+ for this target.\nAll hidden class members will no longer be shown." : "Enable Debug Mode+ for this target.\nAll hidden class members will be shown.", OnDebugModeIconClicked));
			}
			...
			AddHeaderToolbarControl(HeaderPartDrawer.Create(HeaderPart.Base, false, false, baseTooltip, null));
			...
			if(HasEnabledFlag)
			{
				...
				AddHeaderToolbarControl(HeaderPartDrawer.Create(HeaderPart.EnabledFlag, true, true, "Change the enabled state of the Component.", null));
			}
			...
			if(Foldable)
			{
				AddHeaderToolbarControl(HeaderPartDrawer.Create(HeaderPart.FoldoutArrow, true, true, "", OnFoldArrowClicked, OnFoldArrowRightClicked, false));
			}
		...
		private void OpenDocumentation(Event inputEvent)
		{
		...
		private void AddHeaderToolbarControl(HeaderPartDrawer button)
		{
		...
		private void OnFoldArrowClicked(Event inputEvent)
		{
		...
		private void OnFoldArrowRightClicked(Event inputEvent)
		{
			if(Foldable)
			{
				DrawGUI.Use(inputEvent);
				...
				SelectHeaderPart(headerParts.Base);
				var openPosition = FoldArrowPosition;
				openPosition.y += HeaderHeight;
				menu.OpenAt(openPosition);
			}
		}

		private void OnDebugModeIconClicked(Event inputEvent)
		{
			DrawGUI.Use(inputEvent);
			if(debugMode)
			{
		...
		private void OnExecuteMethodIconClicked(Event inputEvent)
		{
			DrawGUI.Use(inputEvent);
		...
		private void OnExecuteMethodIconRightClicked(Event inputEvent)
		{
			DrawGUI.Use(inputEvent);
		...
		{
			#if DEV_MODE && PI_ASSERTATIONS
			Debug.Assert(headerRect.width > 0f, "UpdateHeaderToolbarPositions called with headerRect "+headerRect);
			Debug.Assert(headerRect.height > 0f, "UpdateHeaderToolbarPositions called with headerRect "+headerRect);
			#endif
			...
			for(int n = headerParts.Count - 1; n >= 0; n--)
			{
				...
				switch(part.Part)
				{
					case HeaderPart.FoldoutArrow:
						part.Rect = FoldArrowPosition;
						break;
					case HeaderPart.EnabledFlag:
						part.Rect = EnabledFlagPosition;
						break;
					case HeaderPart.DebugModePlusButton:
						part.Rect = DebugModeIconPosition;
						break;
					case HeaderPart.QuickInvokeMenuButton:
						part.Rect = ExecuteMethodIconPosition;
						break;
					case HeaderPart.ReferenceIcon:
						part.Rect = ReferenceIconPosition;
						break;
					case HeaderPart.ContextMenuIcon:
						part.Rect = ContextMenuIconPosition;
						break;
					#if UNITY_2018_1_OR_NEWER && UNITY_EDITOR
					case HeaderPart.PresetIcon:
						part.Rect = PresetIconPosition;
						break;
					#endif
				}
			}
		...
		private void OnContextMenuIconClicked(Event inputEvent)
		{
			OpenContextMenu(inputEvent, ContextMenuIconPosition, false, (Part)HeaderPart.ContextMenuIcon);
		}
		...
		private void UpdateHeaderButtonPositions(Rect headerRect)
		{
			...
			for(int n = 0, count = headerButtons.Count; n < count; n++)
			{
				...
				if(pos.x > 0f)
				{
					pos.width = Fonts.SmallSizes.GetLabelWidth(button.Label) + 12f; //"minibutton" style
		...
		#if UNITY_EDITOR && !POWER_INSPECTOR_LITE
		...
		protected virtual bool CanPasteFromClipboard()
		{
			#if !POWER_INSPECTOR_LITE
			return Clipboard.CanPasteAs(Type);
			#else
			return false;
			#endif
		}
		...
		protected virtual void AddCopyPasteMenuItems(ref Menu menu)
		{
			...
			if(!ReadOnly)
			{
				if(Clipboard.HasObjectReference())
				{
					...
					{
						#if DEV_MODE && PI_ASSERTATIONS
						Debug.Assert(typeof(IGameObjectDrawer).IsAssignableFrom(typeof(GameObjectDrawer)));
						#endif
		...
		public override bool OnKeyboardInputGiven(Event inputEvent, KeyConfigs keys)
		{
			...
			switch(inputEvent.keyCode)
			{
				...
				case KeyCode.KeypadEnter:
					switch((HeaderPart)SelectedHeaderPart)
					{
						...
						case HeaderPart.EnabledFlag:
							...
							return true;
						case HeaderPart.DebugModePlusButton:
							OnDebugModeIconClicked(inputEvent);
							return true;
						case HeaderPart.QuickInvokeMenuButton:
							OnExecuteMethodIconClicked(inputEvent);
							return true;
						case HeaderPart.ReferenceIcon:
							OpenDocumentation(inputEvent);
							return true;
						case HeaderPart.ContextMenuIcon:
							OnContextMenuIconClicked(inputEvent);
							return true;
						#if UNITY_2018_1_OR_NEWER && UNITY_EDITOR
						case HeaderPart.PresetIcon:
						...
						default:
							#if DEV_MODE
							Debug.LogWarning(Msg("OnKeyboardInputGiven(", inputEvent.keyCode, ") logic for SelectedHeaderPart "+SelectedHeaderPart+" not implemented..."));
							#endif
							return true;
					}
					break;
				...
				case KeyCode.Delete:
					...
					{
						#if DEV_MODE && PI_ASSERTATIONS
						Debug.Assert(typeof(IReorderableParent).IsAssignableFrom(typeof(IGameObjectDrawer)));
						Debug.Assert(typeof(IGameObjectDrawer).IsAssignableFrom(typeof(GameObjectDrawer)));
						#endif
		...
		protected void SelectNextHeaderPartRight(bool moveToNextControlAfterReachingEnd)
		{
			...
			if(next != null)
			{
				SelectHeaderPart(headerParts[next]);
				return;
			}
		...
		public override bool OnClick(Event inputEvent)
		{
			...
			if(mouseoveredPart != null)
			{
				if(mouseoveredPart.OnClicked(inputEvent) && inputEvent.type == EventType.Used)
				{
					return true;
				}
