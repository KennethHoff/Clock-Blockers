...
namespace Sisus
{
	...
	/// </summary>
	[Serializable, DrawerForGameObject(true)]
	public class GameObjectDrawer : ParentDrawer<Component[]>, IGameObjectDrawer, IOnProjectOrHierarchyChanged, IEnumerable<IComponentDrawer>
	{
		...
		private const float tagAndLayerFieldHeight = 18f;
		private const float tagFieldLeftEdgeOffset = 42f;
		private const float tagFieldTopOffset = 33f;
		private const float tagToLayerFieldOffset = 14f;
		private const float layerFieldRightEdgeOffset = 4f;
		private const float tagFieldWiderThanLayerField = 10f;
		#else
		private const float tagAndLayerFieldHeight = 15f;
		private const float tagFieldLeftEdgeOffset = 34f;
		private const float tagFieldTopOffset = 28f;
		private const float tagToLayerFieldOffset = 4f;
		private const float layerFieldRightEdgeOffset = 2f;
		private const float tagFieldWiderThanLayerField = 0f;
		#endif 
		...
		private const float PrefabButtonsLeftPadding = 62f;
		...
		/// <summary>
		/// If true, target will be drawn in grayed out color,
		/// while still remaining editable (i.e. GUI.enabled is true).
		/// This can be used to indicate the target being inactive.
		/// </summary>
		protected bool grayedOut;
		...
		public override bool ReadOnly
		{
			get
			{
				// ReadOnly can get set to true with prefabs when Prefab Quick Editing
				// setting is set to ReadOnly (because GUI.enabled is false).
				// We don't necessarily want this, though, to make sure that the header
				// remains interactive, and the Edit Prefab button can be pressed.
				return false;
			}
		}
		...
		protected float HeaderHeightOutsideDebugMode
		{
			get
			{
				...
				if(isPrefab)
				{
					return DrawGUI.Active.AssetTitlebarHeight(false) + GameObjectHeaderDrawer.OpenInPrefabModeButtonHeight;
				}
				#endif
				return DrawGUI.Active.AssetTitlebarHeight(isPrefabInstance);
				#else
		...
		private Rect NameFieldPosition
		{
			get
			{
				...
				rect.x += 62f;
				rect.y += 9f;
				rect.width = DrawGUI.InspectorWidth - 62f - 74f;
				rect.height = 15f;
				return rect;
			}
		}
		...
		private Rect StaticFlagPosition
		{
			get
			{
				...
				rect.y = labelLastDrawPosition.y + 8f;
				rect.width = 50f;
				rect.height = 16f;
				return rect;
			}
		...
		private Rect DropDownArrowPosition
		{
			get
			{
				...
				rect.x += rect.width + 4f;
				rect.y = labelLastDrawPosition.y + 6f;
				rect.width = 10f;
				rect.height = 14f;
				return rect;
			}
		...
		#if UNITY_EDITOR
		#if UNITY_2018_3_OR_NEWER  // the new prefab workflow was instroduced in version 2018.3
		...
		private float OpenPrefabButtonWidth
		{
			get
			{
				...
				// y = 3 * x + 32
				// x = (y - 32) / 3
				return (lastDrawPosition.width - PrefabButtonsAllPaddings - 32f) / 3f;
			}
		}
		...
		#else // before the new prefab workflow was instroduced in version 2018.3

		/// <summary>
		/// Gets the select prefab button position.
		/// </summary>
		/// <value>
		/// The select prefab button position.
		/// </value>
		private Rect SelectPrefabButtonPosition
		{
			get
			{
				...
				rect.x += 62f;
				rect.y += 48f;
				rect.width = isPrefabInstance ? (DrawGUI.InspectorWidth - 68f) / 3f : 0f;
				rect.height = 15f;
				return rect;
			}
		}
		...
		/// <summary>
		/// Gets the revert prefab button position.
		/// </summary>
		/// <value>
		/// The revert prefab button position.
		/// </value>
		private Rect RevertPrefabButtonPosition
		{
			get
			{
				var rect = SelectPrefabButtonPosition;
		...
		/// <summary>
		/// Gets the apply prefab button position.
		/// </summary>
		/// <value>
		/// The apply prefab button position.
		/// </value>
		private Rect ApplyPrefabButtonPosition
		{
			get
			{
				var rect = RevertPrefabButtonPosition;
				rect.x += rect.width + 1f;
				rect.width -= 6f;
				return rect;
			}
		...
		public virtual void Setup([NotNullOrEmpty]GameObject[] setTargets, [CanBeNull]IParentDrawer setParent, [NotNull]IInspector setInspector)
		{
			...
			if(firstTarget != null)
			{
				...
				#else
				var prefabType = PrefabUtility.GetPrefabType(firstTarget);
				switch(prefabType)
				{
                    ...
                    case PrefabType.ModelPrefab:
						...
						for(int n = targetCount - 1; n >= 1; n--)
						{
							prefabType = PrefabUtility.GetPrefabType(setTargets[n]);
			...
			headerDrawer.SetTargets(setTargets);
			...
			{
				#if UNITY_2018_3_OR_NEWER
				Editors.GetEditor(ref previewEditor, targets, Types.GetInternalEditorType("UnityEditor.GameObjectInspector"), true);
				#else
				Editors.GetEditor(ref previewEditor, targets);
				#endif
			}
			#endif
		}
		...
		protected override void OnLayoutEvent(Rect position)
		{
			...
			else if(RevertPrefabButtonPosition.Contains(mousePos))
			{
				mouseoveredPart = GameObjectHeaderPart.RevertPrefab;
			}
			else if(ApplyPrefabButtonPosition.Contains(mousePos))
			{
				mouseoveredPart = GameObjectHeaderPart.ApplyPrefab;
			}
			#endif
			#endif
			else
		...
		public override bool DrawPrefix(Rect position)
		{
			...
			{
			#endif
				headerDrawer.OnProjectOrHierarchyChanged(targets);
			}
		...
		public override void DrawSelectionRect()
		{
			...
			switch(selectedPart)
			{
				case GameObjectHeaderPart.DropDownArrow:
					DrawGUI.DrawControlSelectionIndicator(DropDownArrowPosition, localDrawAreaOffset);
					return;
				#if UNITY_EDITOR
				...
				case GameObjectHeaderPart.PrefabOverrides:
					...
					return;
				#else
				case GameObjectHeaderPart.RevertPrefab:
					DrawGUI.DrawControlSelectionIndicator(RevertPrefabButtonPosition, localDrawAreaOffset);
					return;
				case GameObjectHeaderPart.ApplyPrefab:
					DrawGUI.DrawControlSelectionIndicator(ApplyPrefabButtonPosition, localDrawAreaOffset);
					return;
				#endif
				#endif
			}
		...
		private void DeepCopyUsingMenuItem()
		{
			...
			var manager = Manager;
			var focusedControl = manager.FocusedControl;
		...
		private void UpdateGrayedOut()
		{
			...
			if(gameObject != null)
			{
				...
				if(isPrefab)
				{
					grayedOut = inspector.Preferences.prefabQuickEditing == PrefabQuickEditingSettings.Enabled && !gameObject.ActiveInPrefabHierarchy();
					onlyHeaderGrayedOut = false;
					#if DEV_MODE && DEBUG_GRAYED_OUT
					Debug.Log(Msg(ToString()+".grayedOut = ", grayedOut, " for prefab with prefabQuickEditing=", inspector.Preferences.prefabQuickEditing, " ActiveInPrefabHierarchy=", gameObject.ActiveInPrefabHierarchy()));
					#endif
					return;
				}
				#endif

				grayedOut = !gameObject.activeInHierarchy && inspector.Preferences.drawInactivateGreyedOut != GreyOut.None;
				onlyHeaderGrayedOut = grayedOut && inspector.Preferences.drawInactivateGreyedOut == GreyOut.HeaderOnly;
				#if DEV_MODE && DEBUG_GRAYED_OUT
				Debug.Log(Msg(ToString()+".grayedOut = ", grayedOut, " for non-prefab with activeInHierarchy=", gameObject.activeInHierarchy));
				#endif
			}
			else
			{
				#if DEV_MODE
				Debug.Log(Msg(ToString()+".UpdateGrayedOut called with GameObject ", null, ". inactive=", inactive));
				#endif
				grayedOut = false;
				onlyHeaderGrayedOut = false;
			}
		...
		{
			var focusedControl = InspectorUtility.ActiveManager.FocusedControl;
				...
				case GameObjectHeaderPart.LayerField:
					if(moveToNextControlAfterReachingEnd)
					{
						...
						if(isPrefabInstance)
						{
							...
							#else
							SelectSubControl(GameObjectHeaderPart.SelectPrefab, ReasonSelectionChanged.SelectNextControl);
							#endif
							return;
						}
						#endif
			...
			if(isPrefabInstance)
			{
				switch(selectedPart)
				{
					...
					case GameObjectHeaderPart.TagField:
						...
						#else
						SelectSubControl(GameObjectHeaderPart.SelectPrefab, ReasonSelectionChanged.SelectControlDown);
						#endif
						return;
					case GameObjectHeaderPart.LayerField:
						#if UNITY_2018_3_OR_NEWER
						...
						#else
						SelectSubControl(GameObjectHeaderPart.RevertPrefab, ReasonSelectionChanged.SelectControlDown);
						#endif
						return;
				}
			}
		...
		public override bool OnClick(Event inputEvent)
		{
			...
			if(mouseoveredPart == GameObjectHeaderPart.Base)
			{
		...
		private GameObjectHeaderPart LastHeaderPart()
		{
			...
			#else
			return isPrefabInstance ? GameObjectHeaderPart.ApplyPrefab : GameObjectHeaderPart.LayerField;
			#endif
			#else
			return GameObjectHeaderPart.LayerField;
			#endif
		}
		...
		public override void UpdateCachedValuesFromFieldsRecursively()
		{
			...
			{
				#if DEV_MODE && PI_ASSERTATIONS
				Debug.Assert(updateCachedValuesFor[lastUpdateCachedValuesMemberIndex].GetType() != typeof(MissingScriptDrawer));
				#endif
				updateCachedValuesFor[lastUpdateCachedValuesMemberIndex].UpdateCachedValuesFromFieldsRecursively();
			}
			catch(NullReferenceException)
			{
				#if DEV_MODE
				...
				if(requireComponent != null)
				{
					...
					if(requiredType != null)
					{
						...
						if(requiredType.IsAbstract || requiredType.IsBaseComponentType())
						{
							...
							requiredComponents.Add(new AnyType(requireAny));
				...
				if(requireComponents != null)
				{
					...
					else
					{
						...
						if(types.Length == 1)
						{
							...
							{
								types = TypeExtensions.GetExtendingComponentTypes(test, false);
							}
						}
