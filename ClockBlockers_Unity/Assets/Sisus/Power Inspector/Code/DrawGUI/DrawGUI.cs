...
namespace Sisus
{
	public abstract class DrawGUI
	{
		...
		public const float MinAutoSizedPrefixLabelWidth = 35f; // Changed to 35 from 78 so that if transform member labels are configured to be "P", "R" and "S" then that space will be optimized.
		public const float MinControlFieldWidth = 74f;
		...
		#if UNITY_EDITOR
		public static Color PlayModeColorOverlay = new Color(0f, 0f, 0f, 0.2f);
		#endif
		...
		public static bool EditingTextField
		{
			...
			set
			{
				...
				if(InspectorUtility.ActiveManager != null && InspectorUtility.ActiveManager.SelectedInspector != null)
				{
					...
					{
						#if DEV_MODE && PI_ASSERTATIONS
						if(EditorWindow.focusedWindow != null && !ReferenceEquals(EditorWindow.focusedWindow, InspectorUtility.ActiveInspectorDrawer) && !PopupMenu.isOpen)
						{ Debug.LogError("EditingTextField was set with focusedWindow ("+ EditorWindow.focusedWindow + ") != InspectorUtility.ActiveDrawer ("+ InspectorUtility.ActiveInspectorDrawer + ")"); }
						#endif
		...
		private static void DoSetup([NotNull]InspectorPreferences preferences)
		{
			...
			{
				PlayModeColorOverlay = setPlayModeTint;
			}
			#endif
		...
		public static void BeginOnGUI(InspectorPreferences preferences, bool editorMode)
		{
			...
			if(editorMode)
			{
				Platform.Active = Platform.Editor;
			}
			else
			#endif
			{
				Platform.Active = Platform.Runtime;
			}
			Setup(preferences);
		...
		public void ColorRect(Rect position, Color color)
		{
			...
			var colWas = GUI.color;
			GUI.color = color;
			// Using GUI.Label instead of DrawTexture, so that it is automatically affected by play mode tint
			GUI.Label(position, GUIContent.none, InspectorPreferences.Styles.WhiteTexture);
			GUI.color = colWas;
		}
		
		public void DrawTexture(Rect position, Texture texture)
		{
			if(Event.current.type != EventType.Repaint)
			{
				return;
			...
			#if UNITY_EDITOR
			HandleTintForEditorPlayMode(position);
			#endif
		}

		#if UNITY_EDITOR
		public void HandleTintForEditorPlayMode(Rect position)
		{
			if(EditorApplication.isPlayingOrWillChangePlaymode)
			{
				TintForEditorPlayMode(position);
			}
		}

		public void TintForEditorPlayMode(Rect position)
		{
			var colWas = GUI.color;
			GUI.color = PlayModeColorOverlay;
		...
		{
			#if DEV_MODE //wip test
			// new test! makes all selection indicators look more like the toolbar selection indictors
			if(rect.height > 2f)
			{
		...
		{
			GUI.DrawTexture(lineRect, InspectorUtility.Preferences.graphics.WhitePixel);
		}
		...
		{
			#if DEV_MODE && PI_ASSERTATIONS
			if(lineRect.width == 1f) { Debug.Assert(lineRect.height >= 1f); }
			else { Debug.Assert(lineRect.height == 1f);	}
		...
		public void SetCursor(MouseCursor cursor)
		{
			...
			{
				Debug.LogError(InspectorUtility.MouseoveredInspector+".ScreenSpaceWindowRect(" + InspectorUtility.ActiveInspector.State.ScreenSpaceWindowRect + ") did not contain CursorPos "+Cursor.LocalPosition+" with Event="+StringUtils.ToString(Event.current)+ ", MouseButtonIsDown=" + InspectorUtility.ActiveManager.MouseDownInfo.MouseButtonIsDown);
			}
			#endif
		...
		public abstract void MaskPopup(Rect position, IMaskDrawer popupField);
		...
		public abstract void PingObject(Object target);
