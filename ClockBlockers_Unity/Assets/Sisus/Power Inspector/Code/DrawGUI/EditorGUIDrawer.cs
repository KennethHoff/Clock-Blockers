...
namespace Sisus
{
	public sealed class EditorGUIDrawer : DrawGUI
	{
		...
		public const float AssetTitlebarHeightWithOneButtonRow = 53f;
		#else
		public const float AssetTitlebarHeightWithOneButtonRow = 45f;
		#endif
		public const float AssetTitlebarHeightWithTwoButtonRows = AssetTitlebarHeightWithOneButtonRow + 20f;
		...
		public override DragAndDropVisualMode DragAndDropVisualMode
		{
			get
			{
				return FromEditorDragAndDropVisualMode(DragAndDrop.visualMode);
			}

			set
			{
				#if DEV_MODE && DEBUG_SET_DRAG_N_DROP_VISUAL_MODE
				if(DragAndDrop.visualMode != ToEditorDragAndDropVisualMode(value)) { Debug.Log("DragAndDrop.visualMode = "+value); }
				#endif

				DragAndDrop.visualMode = ToEditorDragAndDropVisualMode(value);
			}
		}
		...
		public override bool Foldout(Rect position, GUIContent label, bool unfolded, GUIStyle guiStyle, Rect? highlightRect = null)
		{
			...
			EditorGUI.indentLevel = 0;
			{
				...
				//it won't clip. This is needed e.g. when drawing on top of the verical splitter / resizer control.
		...
		/// </returns>
		public static Rect AssetHeader(Rect position, [NotNull]Editor editor)
		{
			if(position.Contains(Event.current.mousePosition) && ((Event.current.type == EventType.MouseDown && Event.current.button == 1) || Event.current.type == EventType.ContextClick))
			{
				#if DEV_MODE
			...
			GUILayout.BeginArea(position);
			{
				...
				if(Event.current.type != EventType.Ignore)
				{
					try
					{
						...
						{
							actualDrawnRect = GUILayoutUtility.GetLastRect();
						}
					}
					catch(Exception e)
					{
		...
		public override Gradient GradientField(Rect position, Gradient value)
		{
			...
			EditorGUI.indentLevel = 0;
			{
				...
				#else
				value = typeof(EditorGUI).GetMethod("GradientField", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Static, null, System.Reflection.CallingConventions.Any, GradientFieldMethodParameterTypes, null).InvokeWithParameters(null, position, value) as Gradient;
				#endif
			}
			EditorGUI.indentLevel = indentWas;
			return value;
		...
		public override bool Button(Rect position, GUIContent label)
		{
			return Runtime.Button(position, label);
		}

		public override bool Button(Rect position, GUIContent label, GUIStyle guiStyle)
		{
			bool clicked = Runtime.Button(position, label, guiStyle);
			return clicked;
		}
		...
		public override void EnumFlagsPopup(Rect position, EnumDrawer popupField)
		{
			...
			EditorGUI.indentLevel = 0;
			{
				...
				#else
				popupField.Value = EditorGUI.EnumMaskField(position, popupField.Value);
				#endif
			}
			EditorGUI.indentLevel = indentWas;
		}

		public override void MaskPopup(Rect position, IMaskDrawer popupField)
		{
			int indentWas = EditorGUI.indentLevel;
			EditorGUI.indentLevel = 0;
			{
				position.y += 1f;
				int setValue = EditorGUI.MaskField(position, GUIContent.none, popupField.MaskValue, popupField.ItemNames);
				if(!setValue.Equals(popupField.MaskValue))
				{
					popupField.MaskValue = setValue;
				}
			}
			EditorGUI.indentLevel = indentWas;
		}
		...
		public override string TextArea(Rect position, string value)
		{
			...
			value = EditorGUI.TextArea(position, value, InspectorUtility.Preferences.GUISkin.textArea);
		...
		public UnityEditor.DragAndDropVisualMode ToEditorDragAndDropVisualMode(DragAndDropVisualMode visualMode)
		{
			switch(visualMode)
			{
				...
				case DragAndDropVisualMode.Link:
					return UnityEditor.DragAndDropVisualMode.Link;
				case DragAndDropVisualMode.Generic:
					return UnityEditor.DragAndDropVisualMode.Generic;
				case DragAndDropVisualMode.Move:
					return UnityEditor.DragAndDropVisualMode.Move;
				case DragAndDropVisualMode.Rejected:
					return UnityEditor.DragAndDropVisualMode.Rejected;
				default:
					throw new NotImplementedException("Unrecogined DragAndDropVisualMode has not been implemented yet: "+visualMode);
			}
		}

		public DragAndDropVisualMode FromEditorDragAndDropVisualMode(UnityEditor.DragAndDropVisualMode visualMode)
		{
			switch(visualMode)
			{
				case UnityEditor.DragAndDropVisualMode.None:
					return DragAndDropVisualMode.None;
				case UnityEditor.DragAndDropVisualMode.Copy:
					return DragAndDropVisualMode.Copy;
				case UnityEditor.DragAndDropVisualMode.Link:
					return DragAndDropVisualMode.Link;
				case UnityEditor.DragAndDropVisualMode.Generic:
					return DragAndDropVisualMode.Generic;
				case UnityEditor.DragAndDropVisualMode.Move:
					return DragAndDropVisualMode.Move;
				case UnityEditor.DragAndDropVisualMode.Rejected:
					return DragAndDropVisualMode.Rejected;
				default:
					#if DEV_MODE
					Debug.LogWarning("Unrecogined DragAndDropVisualMode has not been implemented yet: "+visualMode);
					#endif
					return (DragAndDropVisualMode)visualMode;
			}
		}
		...
		public override void AddCursorRect(Rect position, MouseCursor cursor)
		{
			...
			#endif
			EditorGUIUtility.AddCursorRect(position, ToEditorCursor(cursor));
		}
		
		public UnityEditor.MouseCursor ToEditorCursor(MouseCursor cursor)
		{
			switch(cursor)
			{
				case MouseCursor.Arrow:
					return UnityEditor.MouseCursor.Arrow;
				case MouseCursor.Text:
					return UnityEditor.MouseCursor.Text;
				case MouseCursor.ResizeVertical:
					return UnityEditor.MouseCursor.ResizeVertical;
				case MouseCursor.ResizeHorizontal:
					return UnityEditor.MouseCursor.ResizeHorizontal;
				case MouseCursor.Link:
					return UnityEditor.MouseCursor.Link;
				case MouseCursor.SlideArrow:
					return UnityEditor.MouseCursor.SlideArrow;
				case MouseCursor.ResizeUpRight:
					return UnityEditor.MouseCursor.ResizeUpRight;
				case MouseCursor.ResizeUpLeft:
					return UnityEditor.MouseCursor.ResizeUpLeft;
				case MouseCursor.MoveArrow:
					return UnityEditor.MouseCursor.MoveArrow;
				case MouseCursor.RotateArrow:
					return UnityEditor.MouseCursor.RotateArrow;
				case MouseCursor.ScaleArrow:
					return UnityEditor.MouseCursor.ScaleArrow;
				case MouseCursor.ArrowPlus:
					return UnityEditor.MouseCursor.ArrowPlus;
				case MouseCursor.ArrowMinus:
					return UnityEditor.MouseCursor.ArrowMinus;
				case MouseCursor.Pan:
					return UnityEditor.MouseCursor.Pan;
				case MouseCursor.Orbit:
					return UnityEditor.MouseCursor.Orbit;
				case MouseCursor.Zoom:
					return UnityEditor.MouseCursor.Zoom;
				case MouseCursor.FPS:
					return UnityEditor.MouseCursor.FPS;
				case MouseCursor.CustomCursor:
					return UnityEditor.MouseCursor.CustomCursor;
				case MouseCursor.SplitResizeUpDown:
					return UnityEditor.MouseCursor.SplitResizeUpDown;
				case MouseCursor.SplitResizeLeftRight:
					return UnityEditor.MouseCursor.SplitResizeLeftRight;
				default:
					throw new NotImplementedException("Unrecogined MouseCursor has not been implemented yet: "+cursor);
			}
		}
